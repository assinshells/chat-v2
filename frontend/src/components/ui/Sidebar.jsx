...
import { COLOR_OPTIONS, GENDER_OPTIONS } from '../../constants/config';
...
const Sidebar = memo(function Sidebar({ 
  ...
  const handleColorChange = useCallback(async (color) => {
    ...
    try {
      const updatedUser = await updateColor(color, onColorChange);
      if (updatedUser && onColorChange) {
  ...
  const handleGenderChange = useCallback(async (gender) => {
    setSelectedGender(gender);
    try {
      const updatedUser = await updateGender(gender, onGenderChange);
      if (updatedUser && onGenderChange) {
        onGenderChange(updatedUser);
      }
    } catch (error) {
      console.error('Failed to update gender:', error);
    }
  }, [updateGender, onGenderChange]);
